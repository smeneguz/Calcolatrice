...
Table::Table(BusinessLogic* l, QWidget *parent) : QWidget(parent)
{
    log = l;
    ...
    //select Object1D
    for(int i=0; i <log->getVectorSize(0); i++)
        Object1DSelect->addItem(QString::number(i));
    //select Object2D
    for(int i=0; i < log->getVectorSize(1); i++)
        Object2DSelect->addItem(QString::number(i));
    //select Object3D
    for(int i=0; i < log->getVectorSize(2); i++)
        Object3DSelect->addItem(QString::number(i));
    //select Color
    for(int i=0; i < log->getVectorSize(3); i++)
